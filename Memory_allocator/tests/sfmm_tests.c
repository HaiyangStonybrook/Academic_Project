...
Test(sf_memsuite_student, Malloc_great_than_pages, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(PAGE_SZ << 3);
...
Test(sf_memsuite_student, Malloc_over_one_page, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(6144);
	//void* y=sf_malloc(PAGE_SZ);
	...
	*(int*)x = 18;
	...
	cr_assert(*(int*)x == 18, "sf_malloc failed to give proper space for an int of x!");
	//cr_assert(*(int*)y == 36, "sf_malloc failed to give proper space for an int of y!");

	sf_header *header = (sf_header*)((char*)x - 8);
	//sf_header *header_y=(sf_header*)((char*)y-8 );

	/* There should be one block of size 2048 in list 3 */
	free_list *fl = &seg_free_list[find_list_index_from_size(PAGE_SZ*2 - (header->block_size << 4))];

	cr_assert_not_null(fl, "Free list is null");

	cr_assert_not_null(fl->head, "No block in expected free list!");
	cr_assert_null(fl->head->next, "Found more blocks than expected!");
	cr_assert(fl->head->header.block_size << 4 == 2032);
	cr_assert(fl->head->header.allocated == 0);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
	cr_assert(get_heap_start() + PAGE_SZ*2 == get_heap_end(), "Allocated more than necessary!");
}


Test(sf_memsuite_student, consistent_free_coalesce, .init = sf_mem_init, .fini = sf_mem_fini ) {
	sf_errno = 0;
	sf_malloc(sizeof(double));
	void *w =  sf_malloc(sizeof(long));//32
	void *x =  sf_malloc(sizeof(double) * 11); //88+16=112
	void *y =  sf_malloc(sizeof(char)); //32
	void *z =  sf_malloc(sizeof(int)*16); //64+16 = 80
	...
	free_list *fl_z = &seg_free_list[find_list_index_from_size(80)];
	free_list *fl_y = &seg_free_list[find_list_index_from_size(112)];
	free_list *fl_x = &seg_free_list[find_list_index_from_size(224)];
	free_list *fl_w = &seg_free_list[find_list_index_from_size(256)];

	cr_assert_null(fl_z->head, "Unexpected block in list fl_w!");
	cr_assert_null(fl_y->head, "Unexpected block in list fl_y!");
	cr_assert_not_null(fl_x->head, "Unexpected block in list fl_x!");

	cr_assert_not_null(fl_w->head, "Unexpected block in list fl_w!");
	cr_assert_null(fl_w->head->next, "Found more blocks than expected!");
	cr_assert(fl_x->head->header.block_size << 4 == 256);
	cr_assert(fl_x->head->header.allocated == 0);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}

Test(sf_memsuite_student, realloc_same_block, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(int)*16);
	/* void *y = */ sf_malloc(10);
	x = sf_realloc(x, sizeof(int)*16);

	free_list *fl = &seg_free_list[find_list_index_from_size(3984)];

	cr_assert_not_null(x, "x is NULL!");
	cr_assert_not_null(fl->head, "No block in expected free list!");
	cr_assert(fl->head->header.block_size << 4 == 3984, "Free Block size not what was expected!");

	sf_header *header = (sf_header*)((char*)x - 8);
	cr_assert(header->block_size << 4 == 80, "Realloc'ed block size not what was expected!");
	cr_assert(header->allocated == 1, "Allocated bit is not set!");
}


Test(sf_memsuite_student, realloc_block_is_zero, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(double) * 8);
	void *y = sf_realloc(x, 0);

	cr_assert_null(y, "y is NULL!");
