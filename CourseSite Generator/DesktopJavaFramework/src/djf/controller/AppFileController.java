...
public class AppFileController {
    ...
    public void handleExportRequest()throws IOException{
     ...
     /*   PropertiesManager props = PropertiesManager.getPropertiesManager();
        AppMessageDialogSingleton dialog = AppMessageDialogSingleton.getSingleton();
       String fileDir = System.getProperty("user.dir"); ///Users/hy/workspace/NetBeans/CSE219/hw2/TAManager_Solution
       String abPath =null;
       int index = fileDir.lastIndexOf("TAManager_Solution"); // index of "T"
       String fileDir_1 = fileDir.substring(0, index);
       String sourceDir =   fileDir_1+"TAManagerTester/public_html/";
       FileChooser fc = new FileChooser();
	fc.setInitialDirectory(new File(PATH_WORK));
	fc.setTitle(props.getProperty(EXPORT_TITLE));
        fc.setInitialFileName("public_html");
	File selectedFile = fc.showSaveDialog(app.getGUI().getWindow());
        if(selectedFile!=null){
             try {
                    abPath = selectedFile.getAbsolutePath();
                    File dst = new File(abPath);
                    String sourceCss = sourceDir+"css/"; // directory of css
                    File sou = new File(sourceCss);
                    FileUtils.copyDirectoryToDirectory(sou, dst);

                    String sourceImage = sourceDir+"images/"; // directory of images
                    sou = new File(sourceImage);
                    FileUtils.copyDirectoryToDirectory(sou, dst);

                    String sourceJs = sourceDir+ "js/";     // directory of js
                    sou = new File(sourceJs);
                    FileUtils.copyDirectoryToDirectory(sou, dst);

                    String sourceSyllabus = sourceDir+ "syllabus.html/";     // directory of syllabus
                    sou = new File(sourceSyllabus);
                    FileUtils.copyFileToDirectory(sou, dst);
            ...
            if(abPath!=null){
              String sourceJsOH = abPath+ "/js/OfficeHoursGridData.json";
              File officeHour = new File(sourceJsOH);
            try {
                
                app.getFileComponent().saveData(app.getDataComponent(), officeHour.getPath());
            } catch (IOException ex) {
                Logger.getLogger(AppFileController.class.getName()).log(Level.SEVERE, null, ex);
            }
    }*/
